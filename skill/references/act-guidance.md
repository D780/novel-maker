...
## `/novel-weaver act` 交互流程

### 阶段一：展示完整上下文

用户输入 `/novel-weaver act` 后，AI 首先读取并展示以下信息，帮助用户判断全局位置：
...
准备好了就告诉我：/novel-weaver write 写第X章
```
...
在分段的段间，AI 自动触发简化版 `/novel-weaver act`：
...
每次 `/novel-weaver write` 完成后执行：

```
字数检查：XX字
├── < 2200字 → ⚠ 严重不足！自动提示 /novel-weaver expand
├── 2200-2800字 → ✅ 符合要求
