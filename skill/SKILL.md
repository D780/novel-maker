...
本项目依赖 `skill/scripts/` 下的 Python 脚本进行自动化检查。首次使用前必须安装：

```bash
# 方式一：运行安装脚本
python skill/scripts/common/install.py

# 方式二：手动安装依赖
pip install -r skill/scripts/common/requirements.txt
```

安装后可通过以下命令验证：

```bash
python skill/scripts/writer/check_wordcount.py 测试文件.md
```
...
| **每50章** | 生成大总结 | ✅ 必须 |
| **每幕** | 更新 subplot-board.md、规划下一幕 | ✅ 必须 |
...
- [ ] 生成/更新 `.novel-maker/chapters/chXX-summary.md`
- [ ] 写手输出自检报告
- [ ] 审计师输出审计报告
- [ ] 修订师修复 P0/P1 问题
```
...
- [ ] 完整 33 维度审查
- [ ] 更新 `.novel-maker/truth/characters.md`
- [ ] 更新 `.novel-maker/truth/timeline.md`
- [ ] 更新 `.novel-maker/truth/emotional-arcs.md`

#### 每10章必做（规划师/复盘师）

- [ ] 生成小总结 `.novel-maker/summaries/summary-chXX.md`
- [ ] 对比 `plan.md` 与实际剧情，检查规划偏离
- [ ] 若偏离，提示用户："当前剧情与 plan.md 第X幕规划不一致，是否更新计划？"

#### 每幕必做（规划师）

- [ ] 更新 `.novel-maker/truth/subplot-board.md`
- [ ] 反向更新 `plan.md`（添加实际执行列）
- [ ] 规划下一幕

#### 每卷末必做（复盘师）

- [ ] 生成卷总结 `.novel-maker/summaries/volume-XX-summary.md`
- [ ] 更新所有 truth-files
- [ ] 生成读者评论模拟

### 6. Truth-Files 更新清单

每章完成后，复盘师必须检查以下 truth-files：

| 文件 | 触发条件 | 更新内容 |
|------|---------|---------|
| `current-state.md` | 每章 | 世界状态、角色状态、剧情进展 |
| `pending-hooks.md` | 新伏笔/回收伏笔 | 伏笔状态、预计回收章节 |
| `characters.md` | 新角色/角色变化 | 角色档案、关系、成长 |
| `world-setting.md` | 新地点/势力/规则 | 世界观设定、势力分布 |
| `power-system.md` | 力量/等级变化 | 等级体系、能力规则 |
| `timeline.md` | 每5章或时间推进 | 事件时间线 |
| `emotional-arcs.md` | 每5章或情绪转折 | 角色情绪弧线 |
| `subplot-board.md` | 每幕或支线变化 | 支线进度、状态 |

### 7. 规划同步机制

规划不是一次性文档，必须随实际写作同步更新。
...
#### 操作流程
1. 规划师读取当前 `plan.md` 和最近章节摘要
2. 对比原规划与实际执行
3. 若一致：继续执行
4. 若偏离：
   - 标记偏离点
   - 分析偏离原因（用户指令、剧情自然发展、规划不合理）
   - 提出 2-3 个调整方案
   - 等待用户确认后更新 `plan.md`
5. 反向更新 `plan.md` 的"实际执行"列
...
| **写手** | 章节正文生成、文风应用、角色模拟写作、**红线自检** | `/novel-maker write` |
