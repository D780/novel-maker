...
│   ├── references/         # 参考文档（22个，含平台适配规则、编辑视角、爽点追踪）
│   ├── rules/              # 规则文件（4个：反AI表达、角色声音、一致性检查、智能查询）
│   ├── styles/             # 文风库（22位作者，7大流派）
│   ├── templates/          # 模板文件（21个，含剧情卡片、角色语音卡、场景规划卡）
│   ├── genre-packs/        # 题材包（11个题材 + 通用默认）
│   ├── arc-templates/      # 篇章弧线模板（6通用 + 42题材特定）
│   └── scripts/            # 脚本工具（23个，按角色分组：common/writer/auditor/reviewer/planner/coordinator）
├── web/                    # Web UI
│   ├── index.html
│   ├── css/
│   └── js/
